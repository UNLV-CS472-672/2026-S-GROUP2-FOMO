...
| `@fomo/env`     | Typed environment config used by backend + web + mobile |
...
| `packages/backend/.env.example` | Backend – Convex auth + ingestion vars  |
...
For backend Convex deployment variables, also set values with `pnpm exec convex env set ...` from `packages/backend` (or in the Convex dashboard UI).
...
| `CLERK_WEBHOOK_SECRET`                       | Backend | Yes      | Clerk webhook signing secret (`whsec_...`) used by `/clerk-users-webhook`               |
...
| `EXPO_PUBLIC_CLERK_GOOGLE_WEB_CLIENT_ID`     | Mobile  | Yes      | Google OAuth web client ID for Clerk Expo auth                                          |
| `EXPO_PUBLIC_CLERK_GOOGLE_ANDROID_CLIENT_ID` | Mobile  | Yes      | Google OAuth Android client ID for Clerk Expo auth                                      |
| `EXPO_PUBLIC_CLERK_GOOGLE_IOS_CLIENT_ID`     | Mobile  | Yes      | Google OAuth iOS client ID for Clerk Expo auth                                          |
| `EXPO_PUBLIC_CLERK_GOOGLE_IOS_URL_SCHEME`    | Mobile  | Yes      | Google OAuth iOS URL scheme (reversed client ID format)                                 |
