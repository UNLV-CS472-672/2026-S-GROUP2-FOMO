...
def user_exists(user_id: str) -> bool:
    return client.query("data_ml/query:userId", {"userId": user_id}) is not None
...
def join_user_events() -> pd.DataFrame:
    ...
    usersToEvents_data = client.query("data_ml/query:list", {"table_name": "usersToEvents"})
    usersToEvents_df = pd.json_normalize(usersToEvents_data)
    usersToEvents_df = usersToEvents_df[["eventId", "userId"]]

    events_data = client.query("data_ml/query:list", {"table_name": "events"})
...
def raw_matrix_eventTags() -> pd.DataFrame:
    ...
    eventTags_json = client.query("data_ml/query:list", {"table_name": "eventTags"})
    eventTags_df = pd.json_normalize(eventTags_json)
    eventTags_df = eventTags_df[["eventId", "tagId"]]

    tags_json = client.query("data_ml/query:list", {"table_name": "tags"})
...
def raw_matrix_postTags() -> pd.DataFrame:
    ...
    users_json = client.query("data_ml/query:list", {"table_name": "users"})
    users_df = pd.json_normalize(users_json)
    users_df = users_df[["_id"]]

    posts_json = client.query("data_ml/query:list", {"table_name": "posts"})
    ...
    postTags_json = client.query("data_ml/query:list", {"table_name": "postTags"})
...
def upsert_friend_recs(sim_scores: pd.DataFrame, userId: str, rec_amt: int):
    ...
    top_sim_scores = [
        ...
        if client.query("data_ml/query:friend_exists", {"userAId": userId, "userBId": friendId}) is None
    ]
