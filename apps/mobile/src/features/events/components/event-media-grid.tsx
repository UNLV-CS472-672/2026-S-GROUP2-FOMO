...
function EventMediaTile({ post, cell, onOpenPost }: EventMediaTileProps) {
...
export function EventMediaGrid({ posts, onOpenPost }: EventMediaGridProps) {
      ...
      <View style={{ gap: ROW_GAP }}>
        {rows.map((row, rowIndex) => (
          <View key={rowIndex} className="flex-row" style={{ gap: COL_GAP }}>
            {row.map((post) => (
              <EventMediaTile key={post.id} post={post} cell={cell} onOpenPost={onOpenPost} />
            ))}
          </View>
        ))}
      </View>
